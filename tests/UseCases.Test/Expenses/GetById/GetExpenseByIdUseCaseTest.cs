...
public class GetExpenseByIdUseCaseTest
{
    ...
    public async Task Success()
    {
        ...
        result.Type.Should().Be((CashFlow.Communication.Enums.PaymentType)expense.PaymentType);
    ...
    private GetByIdExpensesUseCase CreateUseCase(User user, Expense? expense = null)
    {
        var repository = new ExpensesReadOnlyRepositoryBuilder().GetById(user, expense).Build();
        var mapper =  MapperBuilder.Build();
        var loggedUser = LoggedUserBuilder.Build(user);

        return new GetByIdExpensesUseCase(repository, mapper, loggedUser);
    }
}
